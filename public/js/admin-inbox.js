...
function renderInbox() {
  ...
  document.getElementById('inbox-list').innerHTML = visibleInbox().map(item => {
    ...
    return `<button class="unified-inbox-row source-${meta.tone} ${item.unread ? 'is-unread' : ''} ${item.key === activeInboxKey ? 'active' : ''}" data-key="${inboxEsc(item.key)}" role="option"><span class="inbox-source-icon"><i class="bi ${meta.icon}"></i></span><span class="inbox-row-main"><span class="inbox-row-top"><strong>${inboxEsc(item.name)}</strong><time>${inboxTime(item.created_at)}</time></span><span class="inbox-channel">${meta.label}${item.is_owner ? ' · Owner' : ''}${item.flagged ? ' · Flagged' : ''}${item.state === 'archived' ? ' · Archived' : ''}</span><span class="inbox-preview">${inboxEsc(item.preview)}</span></span></button>`;
...
function transcriptHtml(messages, isOwner = false) { return (messages || []).map(message => `<div class="inbox-transcript ${message.role === 'user' ? 'from-client' : 'from-admin'}"><small>${message.role === 'user' ? (isOwner ? 'Prince Caleb · Owner' : 'Visitor') : 'Lisa'}</small><p>${inboxEsc(message.text)}</p></div>`).join(''); }
...
function scrollReaderToLatest() {
  const reader = document.getElementById('inbox-reader');
  const latest = reader?.querySelector('.inbox-transcript:last-of-type');
  if (!reader || !latest || matchMedia('(max-width: 900px)').matches) return;
  requestAnimationFrame(() => requestAnimationFrame(() => {
    reader.scrollTop = Math.max(0, latest.offsetTop + latest.offsetHeight - reader.clientHeight + 32);
  }));
}
