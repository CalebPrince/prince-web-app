...
function pipelineAgentTimeline(lead) {
  const rows=lead.agent_activity||[];
  if(!rows.length)return `<div class="pipeline-agent-empty"><i class="bi bi-cpu"></i><span><strong>No agent work yet</strong><small>Scheduling a follow-up creates the first durable Nurturer task.</small></span></div>`;
  const icons={queued:'bi-clock-history',leased:'bi-cpu-fill',completed:'bi-check2-circle',failed:'bi-exclamation-triangle',cancelled:'bi-slash-circle'};
  return `<div class="pipeline-agent-timeline">${rows.map(row=>`<article class="agent-event-${pipelineEsc(row.status)}"><span class="pipeline-agent-node"><i class="bi ${icons[row.status]||'bi-circle'}"></i></span><div><header><strong>${pipelineEsc(String(row.kind).replaceAll('_',' '))}</strong><b>${pipelineEsc(row.status)}</b><time>${new Date(String(row.updated_at).replace(' ','T')+'Z').toLocaleString()}</time></header><p>${pipelineEsc(row.outcome||row.last_error||row.reschedule_reason||'Agent work recorded without a note.')}</p><small>${pipelineEsc(row.agent_key)} · attempt ${row.attempts}/${row.max_attempts}${row.status==='queued'?` · due ${new Date(String(row.due_at).replace(' ','T')+'Z').toLocaleString()}`:''}</small></div></article>`).join('')}</div>`;
}
function pipelineDrawer(lead) {
  ...
  <section class="pipeline-detail-section pipeline-agent-section"><div class="pipeline-agent-heading"><div><h5>Agent activity</h5><p>Scheduled work, attempts, and outcomes for this lead.</p></div><a href="/admin/agent-tasks.html">Open queue <i class="bi bi-arrow-up-right"></i></a></div>${pipelineAgentTimeline(lead)}</section>
