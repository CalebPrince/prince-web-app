...
// Traffic lives on the same page as the business report but is driven by its
// own days filter and a separate endpoint (page_views), so it loads on its own.
...
async function loadAnalytics(days = 30) {
  ...
  eventsTbody.innerHTML = (data.top_events || []).length
    ? data.top_events.map(e => `
        <tr>
          <td class="ps-3">${esc(humanizeEventPath(e.path))}</td>
  ...
  referrersTbody.innerHTML = data.top_referrers.length
    ? data.top_referrers.map(r => `
        <tr>
          <td class="ps-3">${esc(r.referrer)}</td>
...
(async function init() {
  ...
  document.getElementById('days-filter').addEventListener('change', (e) => {
    loadAnalytics(Number(e.target.value)).catch(showReportsError);
  });
