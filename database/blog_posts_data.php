...
const INDUSTRY_META = [
    ...
    'technology' => ['label' => 'Technology', 'accent' => '#06b6d4', 'accent_dark' => '#0e7490'],
];
...
TXT,
    ...
    [
        ...
        'excerpt' => 'A practical case study on turning a website live chat from a simple conversation widget into an action-taking assistant that can qualify leads, book calls, and notify Gmail, Slack, Google Sheets, and WhatsApp through Composio.',
        'body' => <<<'TXT'
...
The live chat on my website already had a clear job: help visitors describe what they want to build. But once a good lead came in, the handoff still needed several manual steps: send myself or the client an email summary, notify Slack so the lead is visible immediately, log the lead into a Google Sheet for tracking, send a WhatsApp follow-up or booking confirmation, and create a calendar event when the visitor books a call.
...
I wanted the system to behave like a lightweight operations assistant: understand the visitor's intent, ask for missing details, confirm before taking important actions, save the booking or inquiry inside my app, and send the right updates to the right external tools.
...
In simple terms, the workflow is: visitor message, Live Chat backend, LLM reasoning and tool selection, backend validation, Composio action, then Gmail, Slack, Google Sheets, WhatsApp, or Calendar.
...
For example, the booking workflow can use default actions like GMAIL_SEND_EMAIL, SLACK_SEND_MESSAGE, GOOGLECALENDAR_CREATE_EVENT, and WHATSAPP_SEND_MESSAGE. If a specific Composio dashboard shows a different tool slug, I can override it from settings instead of editing code.
...
For bookings, the backend validates that the visitor gave a real name and email, the date is in the expected format, the time is in the expected format, the slot is still available, and the booking is not a duplicate.
...
When the visitor wants to book a call, the chat guides them through the flow: ask what they want to discuss, check available dates and times, ask for name, email, and optional phone number, confirm the selected slot, create the booking in the app database, then trigger Composio actions after the booking succeeds.
...
After a successful booking, the system can send a Gmail notification with the booking summary, post the booking to Slack, create a Google Calendar event, send a WhatsApp message, and append the lead or booking details to Google Sheets.
...
When a booking or strong lead comes in, I want to see it immediately without checking the admin panel. The Slack message is intentionally concise: who contacted me, what they need, when the call is booked, and how to contact them.
...
The live chat can send structured rows through Composio with fields like date created, name, email, phone, project type, budget range, timeline, lead source, booking date, booking time, conversation summary, and status.
...
Composio let me treat external apps as connected tools: Gmail for email, Slack for team alerts, Google Calendar for events, Google Sheets for lead tracking, and WhatsApp for faster follow-up.
...
The model is helpful for conversation, summarization, and deciding which workflow fits the visitor's intent. But the app still needs deterministic rules for required fields, date and time formats, booking availability, duplicate prevention, placeholder detection, which tools are allowed, and what payload each tool receives.
...
The final setup turns the live chat into an action-taking assistant. Visitors can ask questions and get relevant answers. Qualified leads can be captured from the conversation. Bookings can be created directly from chat. Gmail receives structured booking or lead summaries. Slack gets instant alerts. Google Sheets can track leads in a simple pipeline. WhatsApp can send fast follow-up messages.
