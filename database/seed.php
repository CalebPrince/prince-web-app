...
foreach ($projects as $index => $project) {
    ...
    if ($existingProject) {
        ...
        // Seeded unpublished. These three are illustrative examples with no
        // screenshot, and the homepage gallery they would land in is headed
        // "See what has already shipped": publish one only once it has a real
        // cover image and is genuinely yours to claim. The UPDATE branch above
        // deliberately leaves is_published alone, so publishing one by hand in
        // the admin survives a re-seed. For databases where they are already
        // live, see database/unpublish_sample_projects.php.
        $stmt = $pdo->prepare(
