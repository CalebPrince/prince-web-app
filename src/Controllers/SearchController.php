...
class SearchController
{
    ...
    public static function adminSearch(): void
    {
        ...
        $run("SELECT id,title,summary,category,updated_at FROM projects WHERE title LIKE ? ESCAPE '\\' OR summary LIKE ? ESCAPE '\\' OR case_study_body LIKE ? ESCAPE '\\' ORDER BY updated_at DESC LIMIT 8", [$like,$like,$like], 'Project', '/admin/projects.html', fn($r)=>['title'=>$r['title'],'detail'=>ucwords(str_replace('_',' ',$r['category'])).' - '.mb_substr($r['summary'],0,100),'date'=>$r['updated_at']]);
        $run("SELECT id,invoice_number,client_name,client_email,status,updated_at FROM invoices WHERE invoice_number LIKE ? ESCAPE '\\' OR client_name LIKE ? ESCAPE '\\' OR client_email LIKE ? ESCAPE '\\' OR notes LIKE ? ESCAPE '\\' ORDER BY updated_at DESC LIMIT 8", [$like,$like,$like,$like], 'Invoice', '/admin/invoices.html', fn($r)=>['title'=>$r['invoice_number'].' - '.$r['client_name'],'detail'=>$r['client_email'].' - '.ucfirst($r['status']),'date'=>$r['updated_at']]);
        $run("SELECT m.id,m.client_id,m.body,m.created_at,c.name,c.email FROM client_messages m JOIN clients c ON c.id=m.client_id WHERE m.body LIKE ? ESCAPE '\\' OR c.name LIKE ? ESCAPE '\\' OR c.email LIKE ? ESCAPE '\\' ORDER BY m.created_at DESC LIMIT 8", [$like,$like,$like], 'Client message', '/admin/clients.html', fn($r)=>['title'=>$r['name'],'detail'=>mb_substr($r['body'],0,120),'date'=>$r['created_at'],'href'=>'/admin/clients.html?open='.(int)$r['client_id'].'&tab=messages']);
        $run("SELECT id,client_name,client_email,transcript_json,updated_at FROM chat_sessions WHERE client_name LIKE ? ESCAPE '\\' OR client_email LIKE ? ESCAPE '\\' OR transcript_json LIKE ? ESCAPE '\\' ORDER BY updated_at DESC LIMIT 8", [$like,$like,$like], 'Live chat', '/admin/chats.html', fn($r)=>['title'=>$r['client_name']?:($r['client_email']?:'Chat conversation'),'detail'=>$r['client_email']?:'Visitor conversation','date'=>$r['updated_at'],'href'=>'/admin/chats.html?open='.(int)$r['id']]);
