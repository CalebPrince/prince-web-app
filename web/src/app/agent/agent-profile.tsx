...
export function AgentProfile() {
  ...
  const [agent, setAgent] = React.useState<BuilderOsAgent | null>(null);
  ...
  if (failed) {
    return (
      <div className="agent-profile-error">
        ...
        <h1>That agent dossier could not be opened.</h1>
        <p className="text-muted-custom">Return to the system map and choose a configured specialist.</p>
        <Button variant="brand" size="pill" nativeButton={false} render={<Link href="/builder-os" />}>
    ...
    <>
      <header className="agent-profile-hero">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <p className="agent-profile-command">builder@princecaleb:~$ inspect agent/{agent.key}</p>
          <div className="agent-profile-heading">
            <div>
              <span className="agent-profile-kicker">{agent.key} / configured specialist</span>
              <h1>{agent.name}</h1>
              <p>{agent.role}</p>
            </div>
            <aside className="agent-profile-status">
              <div>
              ...
              <div>
                <span>ACCESS</span>
                <strong>Bounded / monitored</strong>
              </div>
            </aside>
          </div>
        </div>
      ...
      <div className="agent-profile-body">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <section className="agent-profile-section">
            <header>
              <span>01 / MANDATE</span>
              <h2>What {agent.name} is responsible for.</h2>
            </header>
            <div className="agent-profile-copy">{profile.mandate}</div>
          </section>
          <section className="agent-profile-section">
            <header>
              <span>02 / OPERATING SURFACES</span>
              <h2>Where the work becomes visible.</h2>
            </header>
            <div className="agent-capability-grid">
              {profile.surfaces.map((item, index) => (
                <article key={item[0]}>
          ...
          <section className="agent-profile-section">
            <header>
              <span>03 / SIGNAL PATH</span>
              <h2>How one task moves through the system.</h2>
            </header>
            <ol className="agent-signal-path">
              {profile.path.map((item, index) => (
                <li key={item[0]}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <div>
                    <strong>{item[0]}</strong>
          ...
          <div className="agent-profile-cta">
            <div>
              <span className="agent-profile-kicker">DEPLOYMENT PATH</span>
              <h2>Build a system with this responsibility.</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              <Button variant="brand-outline" size="pill" nativeButton={false} render={<Link href="/builder-os" />}>
                Back to network
              </Button>
              <Button
                variant="brand"
                size="pill"
                nativeButton={false}
                render={<Link href={`/contact?agent=${encodeURIComponent(agent.key)}`} />}
                className="group"
              >
                Map this workflow{" "}
